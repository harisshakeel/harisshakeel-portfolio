...
- 3D models: case studies can show an interactive GLB via the optional `model` field on `CaseStudyData`, rendered by `components/ui/model-viewer.tsx` (three.js, dynamically imported, meshopt-compressed GLBs). Web-ready models live in `public/models/`. Raw/unoptimized source exports live in `models/` at the repo root (gitignored — do not commit them). `models/xision.glb` is the ~79 MB raw source export; regenerate the optimized version with `npx @gltf-transform/cli optimize models/xision.glb public/models/xision-avatar.glb --compress meshopt --texture-compress webp --texture-size 1024`.
